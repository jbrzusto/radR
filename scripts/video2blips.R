...
################################################################################
##
## vodep2blips.R:  process frames from a video and create a blips.csv and/or a blipmovie and/or tracks files
##
## This is run from the main radR directory like so:
##
##   cd radR
##   ./rbatch [--no-progress] [--parm PARMFILE] --script scripts/video2blips.R [--no-blips] [--no-blipmovie] [--no-tracks] SITE DIR
##
## where:
##     PARMFILE is a file of configuration parameters
##     SITE is a site code, used in filenames
##     DIR is the input folder of .mp4 videos.
##
...
if(show.progress)
    cat(sprintf("\nProcessing video dir %-63s\n", folder))

vids = dir(folder, pattern="*.mp4$", full.names=TRUE)

## open the video plugin
p = VIDEO$get.ports()[[1]]

for (iv in seq(along=vids)) {
    done.video = FALSE
    config(p, filename=vids[iv])
    start.up(p)
