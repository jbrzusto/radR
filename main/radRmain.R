...
###.endif
      ...
      ## if we have valid data and are either playing or haven't previewed or processed this scan, then
      ## process (possibly) this scan.
...
###.endif
      ...
      ## if not playing, give away some processor cycles

      if (RSS$play.state < RSS$PS$PLAYING) {
        ## resume from paused play?
        if (RSS$play.state == RSS$PS$PAUSED &&
            RSS$resume.from.paused.state &&
            isTRUE(RSS$source$is.live) &&
            ! end.of.data(RSS$source)) {

          ## invoke any hooks (e.g. user might wish to restart learning if enough time
          ## has elapsed); single parameter is elapsed time since last data scan, in seconds
          ## User can cancel resume from pause by returning TRUE from the hook function.
          
          if (! isTRUE(rss.call.hooks(RSS$ONRESUME_PLAY_HOOK, diff(as.numeric(c(RSS$scan.info$timestamp, Sys.time())))))) {
            ## set flag to restart play on
            ## the next iteration (or call to) the event loop
            RSS$new.play.state <- RSS$PS$PLAYING

            ## sync the GUI (because this play event was program-generated)
            rss.gui(START_PLAY)
          }
          
        } else {
          if (!run.once)
          ## no resume from pause
            rss.sleep(RSS$event.loop.sleeptime)
        }
      }
    }, error = function(e) {
      rss.handle.error(geterrmessage())
    })
    if(run.once)
      break
  }
