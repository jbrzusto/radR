 ...
 *
 * RCS: @(#) $Id: tkImgPNG.c 574 2010-05-11 02:07:15Z john $
 */
	...
	Byte* pDest, uLong destSz, uLong* pCRC)
{
	...
	else
	{
		while (destSz)
		{
			int	 blockSz = Tcl_Read(pPNG -> mFile, (void *) pDest, destSz);
...
DecodeLine(Tcl_Interp* interp, PNGImage* pPNG)
{
	...
	if (pPNG -> mBitDepth < 8)
	  bitScale = 255/((1 << pPNG -> mBitDepth)-1);
