...
##                class "other".
##
##  raw-tube: accumulate the maximum value at each pulse/sample
##                location.  This is not, strictly speaking, a "blip
##                trail", but a raw data aggregation which can be used
##                to quickly assess target presence and density,
##                independently of blip finding/filtering parameters.
##
##
...
##
## Optionally, a thumbnail image is written after a fixed interval.
## In raw-tube mode, the accumulated maxes are reset each time this
## happens.  This provides a sequence of images than can easily be examined
## manually to assess target presence/absence and density.
...
get.menus = function() {
    list(
        plugin = list(
            ...
            list (td="gauge",
                  label = "thumbnail image duration (seconds); 0 means none",
                  range = c(0, 36000),
                  increment = 60,
                  value = thumbnail.duration,
                  on.set = function(x) {
                      rss.defer.assign(thumbnail.duration, x, BLIPTRAILS)
                      update()
                  }
                  ),
            "---",
            "Choose thumbnail image folder..." = list ("file",
                  type = "open.dir",
                  title = "Choose a folder for storing thumbnails",
                  on.set = function(f) {thumbnail.folder <<- f},
                  init.file = function() thumbnail.folder
                  ),
            "---",
...
save.thumbnail = function() {
    ...
    fn = file.path(thumbnail.folder, paste0(thumbnail.prefix, format(first.sweep.timestamp, "%Y-%m-%dT%H-%M-%OS3.jpg")))
    jpeg::writeJPEG(x, fn)
}
...
hooks = list(
    ...
    DONE_SCAN = list (enabled = FALSE, read.only = FALSE,
                      f = function(...) {
                          ...
                          ## maybe write a thumbnail image
                          ts = RSS$scan.info$timestamp
                          if (thumbnail.duration > 0 && first.sweep.timestamp > 0 && as.numeric(ts) - as.numeric(first.sweep.timestamp) >= thumbnail.duration) {
                              save.thumbnail()
                              if (current.mode == "raw tube") {
                                  stored.samples[] <<- 0
                              }
                              first.sweep.timestamp <<- 0
                          }
                          if (first.sweep.timestamp == 0) {
                              first.sweep.timestamp <<- ts
                              if (current.mode == "raw tube") {
                                  stored.classes[] <<- RSS$CLASS.VAL$other
                              }
                          }
                          switch(current.mode,
                                 "last N scans" = {
                                 ...
                                 "raw tube" = {
                                     stored.samples[] <<- pmax(stored.samples[], RSS$scan.mat[])
                                 }
                                 )
                      }
                      ),
...
## the timestamp of the first stored sweep
first.sweep.timestamp = 0
