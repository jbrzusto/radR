//  svn: $Id: tilter1.0.h 475 2010-03-11 15:11:54Z john $
...
/**
 * \file 
 * declarations for a minder for version 1.0 of Dave Wilson's antenna tilter
 *
 */
...
/**
 * \brief the class representing the state of the version 1.0 tilter
 *
...
/**
 * \brief the class representing the transition mode for version 1.0 of the tilter
 *
...
/**
 * Typedefs for the template classes, using the tilter 1.0 state and transition mode types.
 * These serve as base classes for our derived classes, which will have the same name, but
 * prefixed with Tilter_1_0_
 *
 */

typedef class minder_pattern_piece <state, trans_mode>	t10_pattern_piece;
typedef class minder_pattern	 <state, trans_mode>	t10_pattern;
typedef class minder_pat_ctrl	 <state, trans_mode>	t10_pat_ctrl;
typedef class minder_hist_entry	 <state, trans_mode>	t10_hist_entry;
typedef class minder_history	 <state, trans_mode>	t10_history;
typedef class minder_device	 <state, trans_mode>	t10_device;
typedef class minder_model	 <state, trans_mode>	t10_model_base;
typedef class minder		 <state, trans_mode>	t10_minder;

/**
 * Class representing a physical model of version 1.0 of Dave Wilson's radar antenna tilter
 *
 */

class t10_model : public t10_model_base {
/**
 ...
 * \verbatim
 ...
 \endverbatim
 *
 *        \li A:  leading edge of start bit for first byte in command
 *        \li B:  just before first calibration step
 *        \li C:  just before 2nd calibration step
 *        \li D:  just after nth calibration step
 *        \li E:  ready to receive another command
 *
 *
 *
 ...
 *
 *        \li A:  leading edge of start bit for first byte in command
 *        \li B:  just before first upward step
 *        \li C:  just before 2nd upward step
 *        \li D:  just after nth upward step
 *        \li E:  ready to receive another command
 *
 *
 ...
 * \verbatim
 ...
 \endverbatim
 *
 *        \li A:  leading edge of stop bit for first byte in command
 *        \li B:  just before first downward step
 *        \li C:  just before 2nd downward step
 *        \li D:  just after nth downward step
 *        \li E:  ready to receive another command
 *
 *  The class constants TC_*_* were obtained by compiling Dave Wilson's code in file
 *
 *  BIRD RADAR LINEARIZED STEP RX - 16_f876_a REV1.0.BAS
 *
...
public:
  ...
  /**
   * version 1.0 of the tilter firmware is not interruptible - a command must
   ...
   * when calibrating.
   */
  virtual inline bool is_state_valid(const state &s) {return s.angle >= 3.0 && s.angle <= 87.0;};
...
protected:
  ...
  */
  ...
  static const double	TC_cal_AB;
  static const double	TC_cal_BC;
  static const double	TC_cal_DE;

  static const double	TC_up_AB;
  static const double	TC_up_BC;
  static const double	TC_up_DE;

  static const double	TC_down_AB;
  static const double	TC_down_BC;
  static const double	TC_down_DE;
  ...
  static const double   speed_cPS;		/**< speed of the communication channel, in chars per second  */
...
typedef struct t10_minder t10_minder;
 ...
 *
 * @return pointer to the t10_minder object, or NULL on error
 */
  t10_minder *t10_get_minder(const char *devname, unsigned max_hist, const char *logname);

/**
 * close a t10_minder device, and stop its controlling thread.
 *
 * @param tm pointer to the t10_minder
 */
void t10_close_minder (t10_minder *tm);

/**
 * set a t10_minder on a linear pattern, with a uniform range of angles each visited for a fixed time
 *
 * @param tm      pointer to a t10_minder
...
/**
 * clear the pattern for a t10_minder.  This will stop any current pattern.
 *
 * @param tm      pointer to a t10_minder
 */
 ...
 * this will resume the pattern where it left off; use t10_restart_pattern to re- run
 ...
 *
 * @param[in] tm pointer to a t10_minder
 ...
 *
 * @param tm pointer to a t10_minder
 * @param at_end one of the codes 0:3 from t10_minder::at_end_do
 */
 ...
 *
 * @param tm pointer to a t10_minder
 * @param dir directon: one of the codes 0:1 from t10_minder::step_dir
 */
