//  svn: $Id: runtilt1.0.c 468 2010-02-26 21:01:17Z john $
...
"
  ...
  t10_minder *tm = t10_get_minder(argv[1], 10, (argc > 2) ? argv[2] : "/dev/stderr" );
...
\n\
   ...
   args.\n"
	...
	total_dur = t10_set_linear_pattern(tm, anglo, angstep, 1 + (int) floor((anghi - anglo)/angstep), dur, speed);
	...
	for (;;) {
	  ...
	  if (sscanf(buf, "%lf %lf %d", &ang, &dur, &speed) < 2)
	    break;
	  t10_add_to_pattern(tm, ang, dur, speed);
	}
	break; 
      }
    case 'c':
      printf("Calibration will complete in %.1f seconds\n", t10_imm_calibrate(tm));
      break;
	...
	t10_imm_set_known_state(tm, ang);
	break;
      }
	...
	t10_imm_tilt_to_angle(tm, ang);
	break;
      }

    case 'n':
      t10_imm_step_pattern(tm);
      break;
	...
	t10_set_dir(tm, (tolower(which) == 'f') ? 1 : 2);
	break;
      }
	...
	t10_get_angles_at_times(tm, bt_lo, bt_step, n_ts, angs);
	...
	t10_set_start_time(tm, n > 0 ? now + start : NOT_A_DATE_TIME);
	t10_set_stop_time(tm, n > 1 ? now + start + end : NOT_A_DATE_TIME);
	t10_run_pattern(tm);
      }
      break;

    case 'R':
      t10_imm_restart_pattern(tm);
      break;
