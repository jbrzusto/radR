//  svn: $Id: tiltserv1.0.cc 612 2010-06-26 13:38:11Z john $
...
where\n\
  ...
  try { // catch server creation errors 
    ...
    t10_minder tm(new t10_device(std::string(argv[1])), new t10_model(), new t10_history(10, (argc > 3) ? argv[3] : "/dev/stderr"));
    ...
    for (;;) {
      ...
      DO_OUTPUT("Tilter control server 1.0 - Enter ? for help\n" << std::setprecision(14));  // high precision for timestamps
...
'[' and ']' show range of args.\n");
	  ...
	  case 'e':
		...
		throw std::runtime_error("must specify mode 1, 2, 3, or 4");
	      tm.set_at_end_do((t10_pat_ctrl::at_end_do) which);
	      break;
	    }
	  ...
	  case 'd':
		...
		throw std::runtime_error("must specify direction F or B");
	      tm.set_step_dir(which == 'f' ? t10_pat_ctrl::FORWARD : t10_pat_ctrl::BACKWARD);
	      break;
	    }
